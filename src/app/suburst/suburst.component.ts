...
@Component({
    ...
    templateUrl: './suburst.component.html',
    styleUrls: ['./suburst.component.scss']
})
export class SuburstComponent implements OnInit, AfterViewInit {
    static that;
    ...
    charts(): void {
        const treemap = dt => d3.treemap()
            .tile(this.tile)
            (d3.hierarchy(dt)
                .sum(d => d.value)
                .sort((a, b) => b.value - a.value));

        this.name = d => d.ancestors().reverse().map(d => d.data.name).join('/');
        ...
        this.group = this.svg.append('g')
            .call(this.render, treemap(this.data));
    }
    ...
    private render(group, root): void {
        const width = 952;
        const height = 700;
        ...
        node.filter(d => d === root ? d.parent : d.children)
            .attr('cursor', 'pointer')
            .on('click', d => {
                return typeof d.target.__data__ === 'string' ? SuburstComponent.that.zoomout(root) : SuburstComponent.that.scalein(d.target.__data__);
            });
